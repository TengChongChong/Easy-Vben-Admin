<template>
  <PageWrapper>
    <a-card title="在下面输入框输入文本,切换后回来内容会保存" :bordered="false" class="!mb-4">
    ...
    <a-card title="标签页操作" :bordered="false" class="!mb-4">
    ...
    <a-card title="标签页复用超出限制自动关闭(使用场景: 动态路由)" :bordered="false" class="!mb-4">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      function toDetail(index: number) {
        go(`/sample/feat/tabs/detail/${index}`);
      }
      return {
        closeAll,
        closeLeft,
